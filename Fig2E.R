...
###filter the data
zfGenes=datExpr
filter <- apply(zfGenes, 1, function(x) length(x[x>100])>=20)
...
diff_results_homecage_sig = diff_results_homecage[diff_results_homecage$padj<0.05,] #select the signficantly differentially expressed microRNAs
...
filter <- apply(zfGenes, 1, function(x) length(x[x>100])>=16)
filtered <- zfGenes[filter,]
genes <- rownames(filtered)
...
##select the signficantly differentially expressed microRNAs





#get common microRNAs
print(intersect(rownames(diff_results_homecage_sig), rownames(diff_results_learning_sig)))

###Prepare data to plot Fig 2E
common_mirnas = data.frame(intersect(rownames(diff_results_homecage_sig), rownames(diff_results_learning_sig)))
...
##plot Fig 2E
# which(rownames(plot) == "mmu-miR-3969") #expression of this microRNA is extremely high and masks other signals
# plot = plot[-46, ]
